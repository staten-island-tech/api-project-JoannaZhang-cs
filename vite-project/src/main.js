  ...
  try {
    ...
    const response = await fetch(
      `${SEARCH_URL}?q=${encodeURIComponent(
        query
      )}&limit=20&fields=id,title,image_id,artist_title,date_display` //changes what user inputs in search bar into URL query parameter
    );

    if (!response.ok) {
      throw new Error("Search failed"); //jumps to catch if there is error
    }

    const data = await response.json();
    return data.data; // array of artworks //json--> javascript object
  } catch (error) {
    console.error(error);
    return [];
  }
...
function inject(art) {
  ...
  const imgSrc = art.image_id
    ? `https://www.artic.edu/iiif/2/${art.image_id}/full/843,/0/default.jpg` //ternary operators like a boolean - ?true and :false
    : "https://via.placeholder.com/800x600?text=No+Image"; //false - placeholder image if no image available
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  searchInput.addEventListener("input", async (e) => {
    ...
    // Empty search → restore original artworks
    if (query === "") {
      clearCards();
      allArtworks.forEach(inject);
      return;
    }
    ...
    const results = await searchArtworksByTitle(query);

    clearCards();

    if (results.length === 0) {
      document.querySelector(".card").innerHTML =
        "<p class='text-slate-500'>No artworks found.</p>";
      return;
    }

    results.forEach(inject);
  });
});
